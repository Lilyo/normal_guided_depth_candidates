...
class EMDC(nn.Module):
    def __init__(self, depth_pretrained_path=None, depth_norm=True):
        super(EMDC, self).__init__()
        self.depth_norm = depth_norm

        # mobilenet_v2 as encoder
        self.enc = mobilenet_v2(pretrained=True, path=depth_pretrained_path)
        ...
        # add postprocess layer, (Linear Propagation as Spatial Diffusion)
        # total prop_time=21
    ...
    def forward(self, dep, rgb, frame_id=0, out_dep = False):
        self.outputs = {}
        if self.depth_norm:
            bz = dep.shape[0]
        ...
        self.outputs[("depth", frame_id, 0)] = y
        self.outputs[("y_loc", frame_id, 0)] = y_loc
        self.outputs[("y_glb", frame_id, 0)] = y_glb
